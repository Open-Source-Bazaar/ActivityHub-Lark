...
const dialogQRC = new Dialog<DialogQRCProps>(({ defer, title, value, ...props }) => (
  <Modal {...props} show={!!defer} onHide={() => defer?.reject(new DialogClose())}>
...
export const QRCodeButton: FC<QRCodeButtonProps> = ({
  ...
  <>
    <Button {...{ size, variant, disabled }} onClick={() => dialogQRC.open().catch(console.warn)}>
      {children}
    </Button>
