...
import { BITABLE_BASE_ID } from '../../utility/configuration';
...
export class ActivityTableModel extends BiTable() {
  ...
  async getAll(filter?: Filter<BITable>, pageSize?: number) {
    ...
    if (this.loadForm && isEmpty(this.formMap))
      this.formMap = Object.fromEntries(await Array.fromAsync(this.loadFormStream()));
...
export class ActivityModel extends BiDataTable<Activity>() {
  ...
  constructor(appId = BITABLE_BASE_ID, tableId = ACTIVITY_TABLE_ID) {
    super(appId, tableId);
  }
  ...
  get currentMeta() {
    const list =
      this.currentAgenda?.allItems.filter(({ startTime, endTime }) => startTime && endTime) || [];
