...
export class AgendaModel extends BiDataTable<Agenda>() {
  ...
  async getStatistics() {
    ...
    const keynoteSpeechCounts = Object.fromEntries(
      Object.entries(statisticsData).map(([forum, { length }]) => [forum, length]),
    ) as Record<string, number>;

    const mentorOrganizationCounts = Object.values(statisticsData)
      .flatMap(forum => forum.flatMap(session => session.mentorOrganizations as string[]))
      .reduce(
        (counts, organization) => {
  ...
  makeFilter(filter: Filter<Agenda>): string {
  ...
  extractFields({ id, fields }: TableRecord<Agenda>) {
    ...
    return {
      ...
      mentors: (mentors as TableCellRelation[])
        ...
        .flat(),
      mentorOrganizations:
        mentorOrganizations && normalizeTextArray(mentorOrganizations as TableCellText[]),
      mentorPositions: mentorPositions && normalizeTextArray(mentorPositions as TableCellText[]),
      mentorSummaries: mentorSummaries && normalizeTextArray(mentorSummaries as TableCellText[]),
  ...
  async getGroup() {
    return (this.group = groupBy(await this.getAll(), ({ forum }) => forum + ''));
  }
}
