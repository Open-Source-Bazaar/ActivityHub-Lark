...
export class ForumModel extends BiDataTable<Forum>() {
  ...
  extractFields({
    ...
    return {
      ...
      producers: (producers as TableCellRelation[])?.flatMap(({ text_arr }) => text_arr),
      volunteers: (volunteers as TableCellRelation[])?.flatMap(({ text_arr }) => text_arr),
      producerPositions:
        producerPositions && normalizeTextArray(producerPositions as TableCellText[]),
      producerOrganizations:
        producerOrganizations && normalizeTextArray(producerOrganizations as TableCellText[]),
      organizationLink: organizationLink && normalizeTextArray(organizationLink as TableCellText[]),
      organizationSummary:
        organizationSummary && normalizeTextArray(organizationSummary as TableCellText[]),
